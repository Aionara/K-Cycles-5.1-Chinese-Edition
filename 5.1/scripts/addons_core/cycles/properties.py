...
from . import engine
from . import camera
from . import kcycles_lightgroups, kcycles_postfx, kcycles_render_cameras
...
enum_kcycles_performance_mode = (
    ('PREMIUM', "高级 - (模式)", "K-Cycles 较慢的性能，可能获得更高质量"),
    ('STANDARD', "标准 - (模式)", "K-Cycles 标准常规性能，可能获得高质量"),
    ('FAST', "快速 - (模式)", "K-Cycles 最快的性能，质量良好"),
    ('NONE', "无 - (不使用)", "不使用 K-Cycles 性能模式"),
)

enum_kcycles_gpu_boost = (
    ('AUTO', "自动 - (自动加速)", "基于 GPU 硬件能力自动加速"),
    ('MAX', "最大 - (最大加速)", "最大 GPU 加速，使用最多内存，渲染性能最佳"),
    ('HIGH', "高 - (高加速)", "高 GPU 加速，使用更多内存，渲染性能高"),
    ('LOW', "低 - (低加速)", "低 GPU 加速，使用略多内存，渲染性能良好"),
    ('NONE', "无 - (不加速)", "无 GPU 加速，使用最少内存，默认渲染性能"),
)

enum_kcycles_gi_preset = (
    ('USER', "用户 - (GI 设置)", "使用场景全局光照设置"),
    ('HIGH', "高 - (GI 预设)", "应用高质量全局光照预设，渲染性能较慢"),
    ('STANDARD', "标准 - (GI 预设)", "应用标准质量全局光照预设，渲染性能较快"),
    ('LOW', "低 - (GI 预设)", "应用低质量全局光照预设，渲染性能更快"),
    ('LOWEST', "最低 - (GI 预设)", "应用最低质量全局光照预设，渲染性能最快"),
)

enum_kcycles_sampling_preset = (
    ('USER', "用户 - (采样设置)", "使用场景渲染采样设置"),
    ('FINAL HIGH', "最终高 - (采样预设)", "应用最终高质量采样预设，渲染性能最慢"),
    ('FINAL', "最终 - (采样预设)", "应用最终质量采样预设，渲染性能较慢"),
    ('MEDIUM', "中等 - (采样预设)", "应用中等质量采样预设，渲染性能中等"),
    ('DRAFT', "草稿 - (采样预设)", "应用草稿质量采样预设，渲染性能较快"),
    ('PREVIEW', "预览 - (采样预设)", "应用预览质量采样预设，渲染性能较快"),
    ('LOW', "低 - (采样预设)", "应用低质量采样预设，渲染性能更快"),
    ('LOWEST', "最低 - (采样预设)", "应用最低质量采样预设，渲染性能最快"),
)

enum_kcycles_lights_mode = (
    ('LIGHT', "灯光场景模式", "灯光场景模式", "OUTLINER_OB_LIGHT", 0),
    ('LIGHTGROUP', "灯光组模式", "灯光组模式", "EVENT_L", 1),
)
...
def get_lightgroups_list_values_delimeted(self):
    list = kcycles_lightgroups.lightgroups_list_delimeted()
    return list

def set_lightgroups_list_values_delimeted(self, value):
    list = kcycles_lightgroups.apply_lightgroups_list_delimeted(value)
    return list

def update_kcycles_lightgroup_camera_mode(self, context):
    if (bpy.context.scene.cycles.kcycles_lightgroup_camera_mode == True):
        kcycles_lightgroups.bpy.ops.kcycles.lightgroups_camera_sync()
    kcycles_lightgroups.refresh_lightgroup_compositor()
...
@persistent
def kcycles_load_post_handler(dummyArg):
    if (len(bpy.context.view_layer.lightgroups) > 0):
        bpy.context.scene.cycles.kcycles_lightgroup = bpy.context.view_layer.active_lightgroup.name
        kcycles_lightgroups.refresh_lightgroup_lights()
    # keys() needed on uninitialize list pair
    # if (not bpy.context.scene.kcycles_lights.keys()):
        # kcycles_lightgroups.refresh_kcycles_lights()

kcycles_active_camera = ""

@persistent
def on_depsgraph_update(scene, depsgraph):
    global kcycles_active_camera

    if (bpy.context.scene.kcycles_lights):
        for update in depsgraph.updates:
            if (update.is_updated_geometry):
                if (type(update.id) is bpy.types.Collection and update.id.name in bpy.context.scene.kcycles_lights):
                    kcycles_lightgroups.refresh_kcycles_lights()
                    break
    if (scene.camera is not None and kcycles_active_camera != scene.camera.name):
        kcycles_active_camera = scene.camera.name
        if (scene.cycles.kcycles_lightgroup_camera_mode == True):
            kcycles_lightgroups.bpy.ops.kcycles.lightgroups_camera_sync()
        kcycles_lightgroups.refresh_lightgroup_compositor()
        kcycles_postfx.update_use_postfx(None, bpy.context)
        kcycles_render_cameras.update_kcycles_render_output(None, bpy.context);
        scene.cycles.kcycles_lightgroup_change += 1

@persistent
def update_frame_change(scene, depsgraph):
...
def get_kcycles_item_name(self):
    return self.get("name", "")

def set_kcycles_item_name(self, value):
    previous_name = self.get("name")
    if (not value or previous_name == value):
        return
    if (self.get("typ") != None and previous_name != None):
        if (self.get("typ") == "M" or self.get("typ") == "L"):
            if (previous_name in bpy.context.scene.objects):
                bpy.context.scene.objects[previous_name].name = value
        elif (self.get("typ") == "C"):
            if (previous_name in bpy.data.collections):
                bpy.data.collections[previous_name].name = value
        elif (self.get("typ") == "W"):
            for node in bpy.context.scene.world.node_tree.nodes:
                if (node.name == previous_name):
                    node.name = value
    self["name"] = value

def get_kcycles_light_exposure(self):
    return self.get("exposure", 0.0)

def set_kcycles_light_exposure(self, value):
    previous_exposure = self.get("exposure")
    if (not previous_exposure):
        previous_exposure = 0.0
    exposure_change = value - previous_exposure
    if (not kcycles_lightgroups.KCYCLES_OT_light_exposure_step.stop_updates):
        bpy.ops.kcycles.light_exposure_step("INVOKE_DEFAULT", typ = "COLLECTION_DIRECT", name = self.get("name"),
            direct_adjustment = exposure_change)
    self["exposure"] = value

class KCyclesItemList(bpy.types.PropertyGroup):
    typ: bpy.props.StringProperty(name="Type Property", default="")
    name: bpy.props.StringProperty(name="Name Property", default="", get=get_kcycles_item_name, set=set_kcycles_item_name)

class KCyclesLightItemList(bpy.types.PropertyGroup):
    typ: bpy.props.StringProperty(name="Type Property", default="")
    name: bpy.props.StringProperty(name="Name Property", default="", get=get_kcycles_item_name, set=set_kcycles_item_name)
    collapse: bpy.props.BoolProperty(name="Collapse Property", default=False)
    exposure: bpy.props.FloatProperty(name="Exposure", description="Adjust the overall exposure of the light collection",
        min=-100.0, max=100.0, soft_min=-5.0, soft_max=5.0, precision=2, default=0.0, get=get_kcycles_light_exposure,
        set=set_kcycles_light_exposure)
    color_tint: bpy.props.FloatVectorProperty(name="Color Tint", subtype="COLOR", description=
        "Adjust the overall color tint of the light collection", size=3, min=0.0, max=1.0, default=(1.0,1.0,1.0),
        update=kcycles_lightgroups.update_lights_collection_color,)
...
class CyclesRenderSettings(bpy.types.PropertyGroup):
    ...
    kcycles_performance_mode: EnumProperty(
        name="K-Cycles 性能模式",
        description="K-Cycles 性能模式设置",
        items=enum_kcycles_performance_mode,
        default='STANDARD',
    )

    kcycles_gpu_boost: EnumProperty(
        name="K-Cycles GPU 加速",
        description="K-Cycles GPU 加速设置",
        items=enum_kcycles_gpu_boost,
        default='AUTO',
    )

    kcycles_fast_viewport_navigation: BoolProperty(
        name="额外视口更新与导航",
        description="额外视口更新与导航，视口渲染较慢",
        default=False,
    )

    kcycles_gi_preset: EnumProperty(
        name="K-Cycles GI 预设",
        description="应用的 K-Cycles 全局光照预设",
        items=enum_kcycles_gi_preset,
        default='USER',
    )

    kcycles_sampling_preset: EnumProperty(
        name="K-Cycles 渲染采样预设",
        description="应用的 K-Cycles 渲染采样预设",
        items=enum_kcycles_sampling_preset,
        default='USER',
    )

    use_kcycles_denoiser: BoolProperty(
        name="超级降噪器",
        description="启用超级降噪器",
        default=False,
        update=update_render_passes,
    ...
    kcycles_denoiser_multi_pass: BoolProperty(
        name="多通道",
        description="使用多通道降噪以获得最高质量",
        default=True,
    )

    kcycles_denoiser_transmission: BoolProperty(
        name="透射",
        description="当场景中使用玻璃/透明材质时启用",
        default=True,
    )

    kcycles_denoiser_volumetrics: BoolProperty(
        name="体积",
        description="当场景中使用体积效果时启用",
        default=False,
    )

    kcycles_denoiser_emission: BoolProperty(
        name="发光",
        description="当场景中使用发光材质时进行降噪",
        default=False,
    )

    kcycles_denoiser_individual_passes: BoolProperty(
        name="独立通道",
        description="对独立灯光通道进行降噪",
        default=False,
    )

    kcycles_lightgroup_camera_mode: BoolProperty(
        name="相机模式",
        description="每个相机拥有独立设置",
        default=False,
        update=update_kcycles_lightgroup_camera_mode,
    )

    kcycles_viewport_match_output_resolution: BoolProperty(
        name="视口匹配输出分辨率",
        description="视口匹配输出分辨率",
        default=False,
    )

    kcycles_viewport_pan_horizontal: FloatProperty(
        name="水平平移",
        description="视口水平平移",
        min=-2.0, max=2.0, precision=3, step=0.01,
        default=0.0,
    )

    kcycles_viewport_pan_vertical: FloatProperty(
        name="垂直平移",
        description="视口垂直平移",
        min=-2.0, max=2.0, precision=3, step=0.01,
        default=0.0,
    ...
    use_kcycles_lightgroups_compositing: BoolProperty(
        name="灯光组合成",
        description="将所有灯光组与色调映射合成",
        default=False,
    )

    kcycles_lightgroup: StringProperty(
        name="灯光组",
        description="为物体或灯光分配灯光组，"
        "注意名称需要与相应的通道匹配",
        default="",
        update=kcycles_lightgroups.update_lightgroup_lights,
    )

    kcycles_lightgroup_advanced_tonemapping: BoolProperty(
        name="灯光组混合器高级色调映射",
        description="灯光组混合器使用高级色调映射",
        default=False,
    )

    kcycles_lightgroup_solo: BoolProperty(
        name="灯光组独奏",
        description="灯光组处于独奏模式",
        default=False,
        update = kcycles_lightgroups.set_refresh_lightgroup_compositor,
    )

    kcycles_lightgroup_remainder: BoolProperty(
        name="灯光组余量",
        description="包含未在其他灯光组中的其余灯光的余量灯光组",
        default=False,
        update=kcycles_lightgroups.update_lightgroup_remainder,
    )

    kcycles_lightgroup_compositor: BoolProperty(
        name="灯光组合成器",
        description="灯光组处于合成器模式",
        default=False,
        update=kcycles_lightgroups.update_lightgroup_compositor,
    )

    kcycles_refresh_lightgroup_compositor: BoolProperty(
        name="刷新灯光组合成器",
        description="刷新灯光组合成器",
        set=kcycles_lightgroups.set_refresh_lightgroup_compositor,
        get=get_true,
        default=False,
    )

    kcycles_refresh_lightgroup_lights: BoolProperty(
        name="刷新灯光组灯光",
        description="刷新灯光组灯光",
        set=kcycles_lightgroups.set_refresh_lightgroup_lights,
        get=get_true,
        default=False,
    )

    kcycles_lights_list_index: IntProperty(
        name="集合/灯光名称",
        description="集合/灯光名称。\n双击：重命名",
        default=0,
        update=kcycles_lightgroups.update_lights_index,
    )

    kcycles_lights_mode: EnumProperty(
        name="灯光混合模式",
        description="在灯光或灯光组之间选择灯光混合模式",
        items=enum_kcycles_lights_mode,
        default='LIGHT',
        update=kcycles_lightgroups.update_lights_mode,
    )

    kcycles_lights_solo: BoolProperty(
        name="灯光独奏",
        description="集合/灯光处于独奏模式",
        default=False,
        update = kcycles_lightgroups.update_lights_solo_mode,
    )

    kcycles_lights_filter_use_lights: BoolProperty(
        name="灯光",
        description="使用灯光",
        default=True,
    )

    kcycles_lights_filter_use_mesh_lights: BoolProperty(
        name="网格",
        description="使用网格灯光",
        default=True,
    )

    kcycles_lights_filter_use_world: BoolProperty(
        name="世界",
        description="使用世界",
        default=True,
    )

    kcycles_lights_filter_use_collections: BoolProperty(
        name="集合",
        description="使用集合",
        default=True,
    )

    kcycles_lightgroup_object_rename: StringProperty(
        name="灯光组物体重命名",
        description="灯光组物体重命名",
        default="",
        set=kcycles_lightgroups.set_lightgroup_object_rename,
        get=get_true,
    )

    kcycles_refresh_light_shadow_linking: BoolProperty(
        name="刷新灯光与阴影链接",
        description="刷新灯光与阴影链接",
        set=kcycles_lightgroups.set_refresh_light_shadow_linking,
        get=get_true,
        default=False,
    )

    kcycles_lightgroup_change: IntProperty(
        name="灯光组更改",
        description="灯光组更改",
        min=0, max=1024,
        default=0,
    ...
    kcycles_lightgroups_list_values: StringProperty(
        name="灯光组数值列表",
        description="逗号分隔的灯光组数值列表",
        default="",
        get=get_lightgroups_list_values_delimeted,
        set=set_lightgroups_list_values_delimeted,
    )
...
class CyclesWorldSettings(bpy.types.PropertyGroup):
    ...
    lightgroup: StringProperty(
        ...
        default="",
        update=kcycles_lightgroups.update_lightgroup_world,
    )
...
def register():
    ...
    bpy.utils.register_class(KCyclesItemList)
    bpy.utils.register_class(KCyclesLightItemList)
    bpy.utils.register_class(kcycles_postfx.KCyclesPostFXSettings)
    bpy.utils.register_class(kcycles_render_cameras.KCyclesRenderCamerasSettings)
    bpy.utils.register_class(kcycles_render_cameras.KCyclesRenderOutputSettings)
    ...
    bpy.types.Scene.kcycles_postfx = PointerProperty(type=kcycles_postfx.KCyclesPostFXSettings)
    bpy.types.Scene.kcycles_bloom_mask_items = bpy.props.CollectionProperty(type=KCyclesItemList)
    bpy.types.Scene.kcycles_tonemapping_mask_items = bpy.props.CollectionProperty(type=KCyclesItemList)
    bpy.types.Scene.lightgroup_lights =  bpy.props.CollectionProperty(type=KCyclesItemList)
    bpy.types.Scene.kcycles_lights =  bpy.props.CollectionProperty(type=KCyclesLightItemList)
    bpy.types.Scene.kcycles_render_cameras = PointerProperty(type=kcycles_render_cameras.KCyclesRenderCamerasSettings)

    bpy.types.Camera.kcycles_postfx = PointerProperty(type=kcycles_postfx.KCyclesPostFXSettings)
    bpy.types.Camera.kcycles_bloom_mask_items = bpy.props.CollectionProperty(type=KCyclesItemList)
    bpy.types.Camera.kcycles_tonemapping_mask_items = bpy.props.CollectionProperty(type=KCyclesItemList)
    bpy.types.Camera.kcycles_render_output = PointerProperty(type=kcycles_render_cameras.KCyclesRenderOutputSettings)

    bpy.app.handlers.depsgraph_update_post.append(on_depsgraph_update)
    bpy.app.handlers.load_post.append(kcycles_load_post_handler)
    bpy.app.handlers.frame_change_post.append(update_frame_change)

def unregister():
    del bpy.types.Scene.kcycles_postfx
    del bpy.types.Scene.kcycles_bloom_mask_items
    del bpy.types.Scene.kcycles_tonemapping_mask_items
    del bpy.types.Scene.lightgroup_lights
    del bpy.types.Scene.kcycles_lights
    del bpy.types.Scene.kcycles_render_cameras
    del bpy.types.Camera.kcycles_postfx
    del bpy.types.Camera.kcycles_bloom_mask_items
    del bpy.types.Camera.kcycles_tonemapping_mask_items
    del bpy.types.Camera.kcycles_render_output
    ...
    bpy.utils.unregister_class(KCyclesItemList)
    bpy.utils.unregister_class(KCyclesLightItemList)
    bpy.utils.unregister_class(kcycles_postfx.KCyclesPostFXSettings)
    bpy.utils.unregister_class(kcycles_render_cameras.KCyclesRenderCamerasSettings)
    bpy.utils.unregister_class(kcycles_render_cameras.KCyclesRenderOutputSettings)
    bpy.app.handlers.depsgraph_update_post.remove(on_depsgraph_update)
    bpy.app.handlers.load_post.remove(kcycles_load_post_handler)
    bpy.app.handlers.frame_change_post.remove(update_frame_change)
