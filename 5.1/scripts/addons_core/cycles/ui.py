...
from . import kcycles_lightgroups, kcycles_postfx, kcycles_render_cameras, kcycles_playback
...
class CYCLES_RENDER_PT_sampling_viewport(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        heading = layout.column(align=True, heading="噪点阈值")
        ...
        if cscene.use_preview_adaptive_sampling:
            ...
            col.prop(cscene, "preview_samples", text="最大采样")
            col.prop(cscene, "preview_adaptive_min_samples", text="最小采样")
        else:
            layout.prop(cscene, "preview_samples", text="采样")
...
class CYCLES_RENDER_PT_sampling_viewport_denoise(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        sub.prop(cscene, "preview_denoiser", text="降噪器")

        col.prop(cscene, "preview_denoising_input_passes", text="通道")
        ...
        if effective_preview_denoiser == 'OPENIMAGEDENOISE':
            col.prop(cscene, "preview_denoising_prefilter", text="预滤波")
            col.prop(cscene, "preview_denoising_quality", text="质量")

        col.prop(cscene, "preview_denoising_start_sample", text="起始采样")

        if effective_preview_denoiser == 'OPENIMAGEDENOISE':
            row = col.row()
            row.active = has_oidn_gpu_devices(context)
            row.prop(cscene, "preview_denoising_use_gpu", text="使用 GPU")
...
class CYCLES_RENDER_PT_sampling_render(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        heading = layout.column(align=True, heading="噪点阈值")
        row = heading.row(align=True)
        row.active = cscene.kcycles_sampling_preset == "USER"
        row.prop(cscene, "use_adaptive_sampling", text="")
        sub = row.row()
        sub.active = cscene.use_adaptive_sampling and cscene.kcycles_sampling_preset == "USER"
        sub.prop(cscene, "adaptive_threshold", text="")

        col = layout.column(align=True)
        col.active = cscene.kcycles_sampling_preset == "USER"
        if cscene.use_adaptive_sampling:
            col.prop(cscene, "samples", text="最大采样")
            col.prop(cscene, "adaptive_min_samples", text="最小采样")
        else:
            col.prop(cscene, "samples", text="采样")
...
class CYCLES_RENDER_PT_sampling_render_denoise(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column()
        col.active = cscene.use_denoising and not cscene.use_kcycles_denoiser

        sub = col.column()
        sub.active = show_denoise_active(context)
        sub.prop(cscene, "denoiser", text="降噪器")

        col.prop(cscene, "denoising_input_passes", text="通道")
        if cscene.denoiser == 'OPENIMAGEDENOISE':
            col.prop(cscene, "denoising_prefilter", text="预滤波")
            col.prop(cscene, "denoising_quality", text="质量")

        if cscene.denoiser == 'OPENIMAGEDENOISE':
            row = col.row()
            row.active = has_oidn_gpu_devices(context)
            row.prop(cscene, "denoising_use_gpu", text="使用 GPU")
...
class CYCLES_RENDER_PT_sampling_path_guiding(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        col.prop(cscene, "use_surface_guiding", text="表面")
        col.prop(cscene, "use_volume_guiding", text="体积", text_ctxt=i18n_contexts.id_id)
...
class CYCLES_RENDER_PT_sampling_path_guiding_debug(CyclesDebugButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(cscene, "guiding_distribution_type", text="分布类型")
        layout.prop(cscene, "guiding_roughness_threshold")
        layout.prop(cscene, "guiding_directional_sampling_type", text="方向采样类型")
...
class CYCLES_RENDER_PT_sampling_advanced(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        row.active = cscene.kcycles_performance_mode == "NONE"
        row.prop(cscene, "sampling_pattern", text="模式")
        ...
        heading = layout.column(align=True, heading="扰动距离")
        # Tabulated Sobol is used when the debug UI is turned off.
        heading.active = cscene.sampling_pattern == 'TABULATED_SOBOL' and cscene.kcycles_performance_mode == "NONE"
        heading.prop(cscene, "auto_scrambling_distance", text="自动")
        heading.prop(cscene, "preview_scrambling_distance", text="视口")
        heading.prop(cscene, "scrambling_distance", text="倍乘器")
...
class CYCLES_RENDER_PT_sampling_advanced_sample_subset(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        col.prop(cscene, "sample_offset", text="偏移")
        col.prop(cscene, "sample_subset_length", text="长度")
...
class CYCLES_RENDER_PT_sampling_lights(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        sub = col.row()
        sub.prop(cscene, "light_sampling_threshold", text="灯光阈值")
...
class CYCLES_RENDER_PT_subdivision(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        sub.prop(cscene, "dicing_rate", text="渲染细分率")
        sub.prop(cscene, "preview_dicing_rate", text="视口")

        col.separator()

        col.prop(cscene, "offscreen_dicing_scale", text="屏幕外缩放")
...
class CYCLES_RENDER_PT_curves(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column()
        col.prop(ccscene, "shape", text="形状")
        if ccscene.shape == 'RIBBONS':
            col.prop(ccscene, "subdivisions", text="曲线细分")
...
class CYCLES_RENDER_PT_volumes(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        col.prop(cscene, "volume_biased", text="有偏")
        if cscene.volume_biased:
            col.prop(cscene, "volume_step_rate", text="渲染步进率")
            col.prop(cscene, "volume_preview_step_rate", text="视口")

            layout.prop(cscene, "volume_max_steps", text="最大步数")
...
class CYCLES_RENDER_PT_light_paths_max_bounces(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        col.prop(cscene, "max_bounces", text="总计")

        col = layout.column(align=True)
        col.prop(cscene, "diffuse_bounces", text="漫射")
        col.prop(cscene, "glossy_bounces", text="光泽")
        col.prop(cscene, "transmission_bounces", text="透射")
        col.prop(cscene, "volume_bounces", text="体积", text_ctxt=i18n_contexts.id_id)

        col = layout.column(align=True)
        col.prop(cscene, "transparent_max_bounces", text="透明")
...
class CYCLES_RENDER_PT_light_paths_clamping(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        col.prop(cscene, "sample_clamp_direct", text="直接光")
        col.prop(cscene, "sample_clamp_indirect", text="间接光")
...
class CYCLES_RENDER_PT_light_paths_caustics(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        col.prop(cscene, "caustics_reflective", text="反射")
        col.prop(cscene, "caustics_refractive", text="折射")
...
class CYCLES_RENDER_PT_light_paths_fast_gi(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        col.prop(cscene, "fast_gi_method", text="方法")

        if world:
            light = world.light_settings
            col = layout.column(align=True)
            col.prop(light, "ao_factor", text="AO 因子")
            col.prop(light, "distance", text="AO 距离")

        if cscene.fast_gi_method == 'REPLACE':
            col = layout.column(align=True)
            col.prop(cscene, "ao_bounces", text="视口反弹")
            col.prop(cscene, "ao_bounces_render", text="渲染反弹")
...
class CYCLES_RENDER_PT_motion_blur(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        col = layout.column()
        col.prop(rd, "motion_blur_position", text="位置")
        col.prop(rd, "motion_blur_shutter")
        col.separator()
        col.prop(cscene, "rolling_shutter_type", text="卷帘快门")
        sub = col.column()
...
class RENDER_PT_kcycles(CyclesButtonsPanel, Panel):
    bl_label = "K-Cycles"
...
class CYCLES_RENDER_PT_kcycles_mode(CyclesButtonsPanel, Panel):
    bl_label = ""
    ...
    def draw_header(self, context):
        ...
        split.label(text = "K-Cycles 模式")
        split.scale_x = 1.0
        split.prop(cscene, "kcycles_performance_mode", text="")
    ...
    def draw(self, context):
        ...
        col = layout.column(align=True)
        col.prop(cscene, "kcycles_gpu_boost", text="GPU 加速")
        col.prop(cscene, "kcycles_gi_preset", text="全局光照预设")
        col.prop(cscene, "kcycles_sampling_preset", text="采样预设")

        layout.use_property_split = False
        row = layout.row()
        split = row.split(factor = 0.17, align=False)
        split.label(text="")
        split.prop(cscene, "kcycles_fast_viewport_navigation", text="额外视口更新与导航")

        layout.use_property_decorate = False
        layout.use_property_split = True

class CYCLES_RENDER_PT_kcycles_denoiser(CyclesButtonsPanel, Panel):
    bl_label = "超级降噪器"
    bl_parent_id = "RENDER_PT_kcycles"
    bl_options = {'DEFAULT_CLOSED'}
    ...
    def draw_header(self, context):
        ...
        self.layout.prop(cscene, "use_kcycles_denoiser", text="")
    ...
    def draw(self, context):
        ...
        layout.use_property_decorate = False
        cscene = context.scene.cycles
        layout.active = cscene.use_kcycles_denoiser

        row = layout.row()
        split = row.split(factor = 0.12, align=False)
        split.label(text="")
        split.prop(cscene, "denoising_use_gpu", text="使用 GPU")

        row = layout.row()
        split = row.split(factor = 0.12, align=False)
        split.active = not cscene.use_kcycles_lightgroups_compositing
        split.label(text="")
        split.prop(cscene, "kcycles_denoiser_multi_pass", text="多通道降噪（最高质量）")

        row = layout.row()
        split = row.split(factor = 0.07, align=False)
        split.label(text="")
        split.active = cscene.kcycles_denoiser_multi_pass and not cscene.use_kcycles_lightgroups_compositing
        split.prop(cscene, "kcycles_denoiser_transmission", text="透射", toggle=True)
        split.prop(cscene, "kcycles_denoiser_volumetrics", text="体积", toggle=True)
        split.prop(cscene, "kcycles_denoiser_emission", text="发光", toggle=True)

        row = layout.row()
        split = row.split(factor = 0.07, align=False)
        split.active = cscene.kcycles_denoiser_multi_pass and not cscene.use_kcycles_lightgroups_compositing
        if (not cscene.denoising_use_gpu):
            split.label(text="")
            split.prop(cscene, "kcycles_denoiser_individual_passes", text="独立通道")
        layout.use_property_split = True
        col = layout.column(align=True)
        col.prop(cscene, "denoising_input_passes", text="通道")
        col.prop(cscene, "denoising_prefilter", text="预滤波")
...
classes = (
    RENDER_PT_kcycles,
    CYCLES_RENDER_PT_kcycles_mode,
    CYCLES_RENDER_PT_kcycles_denoiser,
    kcycles_lightgroups.CYCLES_RENDER_PT_kcycles_lightgroups_and_linking,
    kcycles_render_cameras.RENDER_PT_kcycles_camera_mode,
    kcycles_postfx.RENDER_PT_kcycles_postfx,
...
def register():
    ...
    for cls in classes:
        register_class(cls)
    kcycles_postfx.register()
    kcycles_lightgroups.register()
    kcycles_render_cameras.register()
    kcycles_playback.register()
...
def unregister():
    ...
    kcycles_postfx.unregister()
    kcycles_lightgroups.unregister()
    kcycles_render_cameras.unregister()
    kcycles_playback.unregister()